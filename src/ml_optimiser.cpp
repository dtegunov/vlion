						...
						for (long int iover_rot = 0; iover_rot < exp_nr_oversampled_rot; iover_rot++)
						{
							...
							for (long int ipart = 0; ipart < mydata.ori_particles[my_ori_particle].particles_id.size(); ipart++)
							{
								...
								if (do_ctf_correction && refs_are_ctf_corrected)
								{
								    ...
								    Complex* ptrFref = Fref.data;
									Complex* ptrFrefctf = Frefctf.data;
								    double* ptrExpLocalFctfs = exp_local_Fctfs[ipart].data;
									FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY_4(Fref)
									{
										_mm256_store_pd((double*)(ptrFrefctf), _avx_complex_mul_scalar_2(ptrFref, ptrExpLocalFctfs));
                                        ptrFrefctf += 2;   ptrFref += 2;   ptrExpLocalFctfs += 2;
                                    ...
                                    Complex* ptrFref = Fref.data;
                                    Complex* ptrFrefctf = Frefctf.data;
                                    float* ptrExpLocalFctfs = exp_local_Fctfs[ipart].data;
                                    FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY_4(Fref)
                                    {
                                        _mm256_store_ps((float*)(ptrFrefctf), _avx_complex_mul_scalar_4(ptrFref, ptrExpLocalFctfs));
                                        ptrFrefctf += 4;   ptrFref += 4;   ptrExpLocalFctfs += 4;
                                    }
#endif
								}
								else
...
#ifndef FLOAT_PRECISION
									__m256d __myscale = _mm256_set1_pd(myscale);
                                    Complex* ptrFrefctf = Frefctf.data;
									FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY_4(Frefctf)
									{
										_mm256_store_pd((double*)ptrFrefctf, _mm256_mul_pd(_mm256_load_pd((double*)ptrFrefctf), __myscale));
										ptrFrefctf += 2;
...
#else
                                    __m256 __myscale = _mm256_set1_ps(myscale);
                                    Complex* ptrFrefctf = Frefctf.data;
                                    FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY_4(Frefctf)
                                    {
                                        _mm256_store_ps((float*)ptrFrefctf, _mm256_mul_ps(_mm256_load_ps((float*)ptrFrefctf), __myscale));
                                        ptrFrefctf += 4;
                                    }
#endif
								}
...
//#define DEBUG_GETALLDIFF2
...
#ifndef FLOAT_PRECISION
												Complex* ptrFrefctf = Frefctf.data;
												Complex* ptrFimgShift = Fimg_shift;
												__m256d __localDiff2 = _mm256_set1_pd(0.0);
												__m256d __localSuma2 = _mm256_set1_pd(0.0);
												FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY_4(Frefctf)
												{
												    __m256d __Frefctf = _mm256_load_pd((double*)ptrFrefctf);
												    __localSuma2 = _mm256_add_pd(__localSuma2, _mm256_mul_pd(__Frefctf, __Frefctf));

												    __m256d __FimgShift = _mm256_load_pd((double*)ptrFimgShift);
												    __localDiff2 = _mm256_add_pd(__localDiff2, _mm256_mul_pd(__Frefctf, __FimgShift));

												    ptrFrefctf += 2;    ptrFimgShift += 2;

												    __Frefctf = _mm256_load_pd((double*)ptrFrefctf);
												    __localSuma2 = _mm256_add_pd(__localSuma2, _mm256_mul_pd(__Frefctf, __Frefctf));

												    __FimgShift = _mm256_load_pd((double*)ptrFimgShift);
												    __localDiff2 = _mm256_add_pd(__localDiff2, _mm256_mul_pd(__Frefctf, __FimgShift));

												    ptrFrefctf += 2;    ptrFimgShift += 2;
												}
												double* localDiff2 = (double*)&__localDiff2;
												diff2 -= localDiff2[0] + localDiff2[1] + localDiff2[2] + localDiff2[3];
												double* localSuma2 = (double*)&__localSuma2;
												suma2 += localSuma2[0] + localSuma2[1] + localSuma2[2] + localSuma2[3];
#else
                                                Complex* ptrFrefctf = Frefctf.data;
                                                Complex* ptrFimgShift = Fimg_shift;
                                                __m256 __localDiff2 = _mm256_set1_ps(0.0f);
                                                __m256 __localSuma2 = _mm256_set1_ps(0.0f);
                                                FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY_4(Frefctf)
                                                {
                                                    __m256 __Frefctf = _mm256_load_ps((float*)ptrFrefctf);
                                                    __localSuma2 = _mm256_add_ps(__localSuma2, _mm256_mul_ps(__Frefctf, __Frefctf));

                                                    __m256 __FimgShift = _mm256_load_ps((float*)ptrFimgShift);
                                                    __localDiff2 = _mm256_add_ps(__localDiff2, _mm256_mul_ps(__Frefctf, __FimgShift));

                                                    ptrFrefctf += 4;    ptrFimgShift += 4;
                                                }
                                                float* localDiff2 = (float*)&__localDiff2;
                                                diff2 -= localDiff2[0] + localDiff2[1] + localDiff2[2] + localDiff2[3] + localDiff2[4] + localDiff2[5] + localDiff2[6] + localDiff2[7];
                                                float* localSuma2 = (float*)&__localSuma2;
                                                suma2 += localSuma2[0] + localSuma2[1] + localSuma2[2] + localSuma2[3] + localSuma2[4] + localSuma2[5] + localSuma2[6] + localSuma2[7];
#endif
											...
											else
											{
												...
												Complex* ptrFrefctf = Frefctf.data;
												Complex* ptrFimgShift = Fimg_shift;
												double* ptrMinvsigma2 = Minvsigma2;
												FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY_4(Frefctf)
												{
												    __m256d __c11 = _mm256_load_pd((double*)ptrFrefctf);
												    __m256d __c21 = _mm256_load_pd((double*)ptrFimgShift);
												    __m256d __diff1 = _mm256_sub_pd(__c11, __c21);
													__diff1 = _mm256_mul_pd(__diff1, __diff1);

												    __m256d __c12 = _mm256_load_pd((double*)(ptrFrefctf + 2));
												    __m256d __c22 = _mm256_load_pd((double*)(ptrFimgShift + 2));
												    __m256d __diff2 = _mm256_sub_pd(__c12, __c22);
													__diff2 = _mm256_mul_pd(__diff2, __diff2);

													// horizontal add, i.e. [0.x + 0.y, 1.x + 1.y, 0.z + 0.w, 1.z + 1.w]
													__m256d __diffsum = _mm256_hadd_pd(__diff1, __diff2);

													// element order needs to match __diffsum
													__m256d __Minvsigma2 = _mm256_setr_pd(ptrMinvsigma2[0], ptrMinvsigma2[2], ptrMinvsigma2[1], ptrMinvsigma2[3]);

													__diffsum = _mm256_mul_pd(__diffsum, __Minvsigma2);

													__localDiff2 = _mm256_add_pd(__localDiff2, __diffsum);

													ptrFrefctf += 4;	ptrFimgShift += 4;	ptrMinvsigma2 += 4;
												}
												double* localDiff2 = (double*)&__localDiff2;
												diff2 += (localDiff2[0] + localDiff2[1] + localDiff2[2] + localDiff2[3]) * 0.5;
#else
                                                __m256 __localDiff2 = _mm256_set1_ps(0.0f);
                                                Complex* ptrFrefctf = Frefctf.data;
                                                Complex* ptrFimgShift = Fimg_shift;
                                                float* ptrMinvsigma2 = Minvsigma2;
                                                FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY_8(Frefctf)
                                                {
                                                    __m256 __c11 = _mm256_load_ps((float*)ptrFrefctf);
                                                    __m256 __c21 = _mm256_load_ps((float*)ptrFimgShift);
                                                    __m256 __diff1 = _mm256_sub_ps(__c11, __c21);
                                                    __diff1 = _mm256_mul_ps(__diff1, __diff1);

                                                    __m256 __c12 = _mm256_load_ps((float*)(ptrFrefctf + 4));
                                                    __m256 __c22 = _mm256_load_ps((float*)(ptrFimgShift + 4));
                                                    __m256 __diff2 = _mm256_sub_ps(__c12, __c22);
                                                    __diff2 = _mm256_mul_ps(__diff2, __diff2);

                                                    // horizontal add for fp32, i.e. [0.x + 0.y, 0.z + 0.w, 1.x + 1.y, 1.z + 1.w, 0...]
                                                    __m256 __diffsum = _mm256_hadd_ps(__diff1, __diff2);

                                                    // element order needs to match __diffsum
                                                    __m256 __Minvsigma2 = _mm256_setr_ps(ptrMinvsigma2[0], ptrMinvsigma2[1], ptrMinvsigma2[4], ptrMinvsigma2[5], 
                                                                                         ptrMinvsigma2[2], ptrMinvsigma2[3], ptrMinvsigma2[6], ptrMinvsigma2[7]);

                                                    __diffsum = _mm256_mul_ps(__diffsum, __Minvsigma2);

                                                    __localDiff2 = _mm256_add_ps(__localDiff2, __diffsum);

                                                    ptrFrefctf += 8;	ptrFimgShift += 8;	ptrMinvsigma2 += 8;
                                                }
                                                float* localDiff2 = (float*)&__localDiff2;
                                                diff2 += (localDiff2[0] + localDiff2[1] + localDiff2[2] + localDiff2[3] + 
                                                          localDiff2[4] + localDiff2[5] + localDiff2[6] + localDiff2[7]) * 0.5f;
#endif
											}
#ifdef TIMING
											// Only time one thread, as I also only time one MPI process
